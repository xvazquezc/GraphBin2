...
def run(args):
    ...
    for label, seq in MinimalFastaParser(contigs_file):
        ...
        contig_num = int(re.search("%s(.*)%s" % (start, end), label).group(1))

        start = "_length_"
        end = "_cov"
        length = int(re.search("%s(.*)%s" % (start, end), label).group(1))

        start = "_cov_"
        end = ""
        coverage = int(float(re.search("%s(.*)%s" % (start, end), label).group(1)))
                ...
                if contig_bin != -1:
                    labelled_nodes.add(
                        (
                            ...
                            depth[active_node],
                            abs(
                                coverages.get(contigs_map[node], 0)
                                - coverages.get(contigs_map[active_node], 0)
                            ),
                        )
                    )
                ...
                if len(BFS_labelled_nodes) > 0:
                    ...
                    # Check if all the BFS_labelled_contigs are in one bin
                        ...
                        if (
                            ...
                            and contig_lengths.get(contigs_map[my_node], 10000) < 10000
                        ):
                            remove_labels[my_node] = my_contig_bin
        ...
        if to_bin in non_isolated_unbinned:
            ...
            for n in assembly_graph.neighbors(to_bin, mode=ALL):
                if n not in binned_contigs:
                    cov_d = abs(coverages.get(contigs_map[n], 0) - coverages.get(contigs_map[to_bin], 0))
    ...
    for i in range(n_bins):
        for contig in bins[i]:
            if contig in non_isolated_set:
                node_cov = coverages.get(contigs_map[contig], 0)
                node_len = contig_lengths.get(contigs_map[contig], 0)
    ...
    for k in range(n_bins):
        for contig in bins[k]:
            output_bins.append([contig_names[contig], bins_list[k]])
...
def is_multi(
    ...
    if contig in non_isolated and contig in binned_contigs:
        contig_bin = contig_bin_map.get(contig, -1) if contig_bin_map is not None else -1
        if contig_bin == -1:
            for n in range(n_bins):
                if contig in bins[n]:
                    contig_bin = n
        ...
        bin_coverages[contig_bin] = bin_coverages[contig_bin] - (
            coverages.get(contigs_map[contig], 0) * contig_lengths.get(contigs_map[contig], 0)
        )
        bin_contig_lengths[contig_bin] = (
            bin_contig_lengths[contig_bin] - contig_lengths.get(contigs_map[contig], 0)
        )
        ...
        for neighbour in neighbours:
            nb = contig_bin_map.get(neighbour, -1) if contig_bin_map is not None else -1
            if nb == -1:
                for n in range(n_bins):
            ...
            if nb != -1:
                ...
                neighbour_bin_coverages[nb].append(coverages.get(contigs_map[neighbour], 0))
        ...
        if zero_bin_count < n_bins - 1:
            ...
            for combination in bin_combinations:
                ...
                cov_diff = abs(comb_cov_total - coverages.get(contigs_map[contig], 0))
            ...
            if (
                ...
                and contig_lengths.get(contigs_map[contig], 0) > 1000
            ):
                # return True
